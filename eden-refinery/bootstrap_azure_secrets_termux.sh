...
echo "EDEN Refinery benchmark bootstrap (Azure OpenAI provider)"
...
echo "Starting 1000-request EDEN Refinery benchmark using Azure OpenAI..."
gh workflow run eden-refinery-benchmark.yml --repo "$REPO" -f requests=1000 -f concurrency=16

echo
echo "Run submitted. Latest runs:"
gh run list --repo "$REPO" --workflow eden-refinery-benchmark.yml --limit 3
